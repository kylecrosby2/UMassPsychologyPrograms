...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\kerry\\Documents\\Kyle Files\\PsychoPyProjects\\posnerFiles\\posner_lastrun.py',
...
trials = data.TrialHandler(nReps=2.0, method='random', 
    ...
    trialList=data.importConditions('C:/Users/kerry/Documents/Kyle Files/PsychoPyProjects/posnerFiles/conditions.xlsx'),
...
for thisTrial in trials:
    ...
    trialComponents = [cue, target, resp]
    ...
    while continueRoutine:
        ...
        # *resp* updates
        if resp.status == NOT_STARTED and t >= 0.7-frameTolerance:
            # keep track of start time/frame for later
    ...
    trials.addData('resp.x', resp.x)
    trials.addData('resp.y', resp.y)
    trials.addData('resp.leftButton', resp.leftButton)
    trials.addData('resp.midButton', resp.midButton)
    trials.addData('resp.rightButton', resp.rightButton)
    trials.addData('resp.time', resp.time)
    trials.addData('resp.clicked_name', resp.clicked_name)
    ...
    # update component parameters for each repeat
    feedbackTxt.setText('RT was: ' + str(round(resp.time[0], 3)))
    # keep track of which components have finished
    ...
    for thisComponent in feedbackComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    trials.addData('feedbackTxt.started', feedbackTxt.tStartRefresh)
    trials.addData('feedbackTxt.stopped', feedbackTxt.tStopRefresh)
    thisExp.nextEntry()
